...
$general_dir = 'res/general-images/';
$folders = ['about-page', 'home-page'];
        ...
        switch ($_POST['action']) {
            ...
            case 'delete':
                if (isset($_POST['filename']) && isset($_POST['folder'])) {
                    ...
                    $filepath = $general_dir . $folder . '/' . $filename;
...
<head>
    ...
    <title>Manage General Images - FC Signs</title>
...
<body>
    ...
    <div class="works-container">
        ...
        <div class="service-tabs">
            <?php foreach ($folders as $index => $folder): ?>
                <div class="service-tab <?php echo $index === 0 ? 'active' : ''; ?>" 
                    ...
                    <?php echo str_replace('-', ' ', ucfirst($folder)); ?>
                </div>
            <?php endforeach; ?>
        </div>
        ...
        <!-- Folder Contents -->
        <?php foreach ($folders as $index => $folder):
            $images = glob($general_dir . $folder . "/*.{" . implode(',', $allowed_types) . "}", GLOB_BRACE);
        ?>
            <div class="folder-section" id="<?php echo $folder; ?>" 
                 style="display: <?php echo $index === 0 ? 'block' : 'none'; ?>">
                <div class="folder-header">
                    <h2 class="folder-title"><?php echo str_replace('-', ' ', $folder); ?></h2>
                    <button class="btn btn-primary btn-sm" onclick="toggleUpload('<?php echo $folder; ?>')">
                        <i class="fas fa-plus"></i> Add Images
                    </button>
                </div>

                <!-- Upload Form -->
                <form class="folder-upload" id="upload-<?php echo $folder; ?>" 
                ...
                <div class="image-grid">
                    <?php foreach ($images as $image): ?>
                        <div class="image-card">
                            <img src="<?php echo $image; ?>" alt="General Image">
